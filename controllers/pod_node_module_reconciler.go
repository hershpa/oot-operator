...
func (pnmr *PodNodeModuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	labelName := daemonset.NodeLabelName(moduleName)
	...
	if !podutils.IsPodReady(&pod) || deleting {
		...
		return ctrl.Result{}, pnmr.addLabel(ctx, nodeName, labelName)
	}
}
